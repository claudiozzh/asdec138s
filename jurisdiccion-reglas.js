...
function reglasRango(t) {
  ...
  // Las Brisas III ya NO se resuelve por rango de manzana: es zona compartida.
  // La división 0-29 / 30-34 que había aquí no reflejaba ningún acuerdo entre
  // los dos establecimientos. Ver ZONAS_COMPARTIDAS.
  return null;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════════════════
// ZONAS DE JURISDICCIÓN COMPARTIDA
//
// Zonas donde dos establecimientos NO se han puesto de acuerdo sobre a quién
// corresponden. NO es un error del sistema: es una disputa institucional
// pendiente de resolver.
//
// Los niños de estas zonas aparecen en el Excel de AMBOS establecimientos,
// marcados como zona compartida. Así ninguna licenciada los pierde de vista
// y ambas ven que la asignación es provisional.
//
// Cuando se llegue a un acuerdo, se mueve la zona a JURIS_EXACTA y se quita
// de aquí. Es el único lugar que hay que tocar.
// ═══════════════════════════════════════════════════════════════════════════════
const ZONAS_COMPARTIDAS = {
  'LAS BRISAS III': {
    jurisdicciones: ['PAMPA INALAMBRICA', 'LOS ANGELES'],
    motivo: 'C.S. Pampa Inalámbrica y P.S. Los Ángeles no se han puesto de acuerdo: '
          + 'uno sostiene que la zona se divide por manzanas, el otro que le '
          + 'corresponde completa. Manzanas existentes: 26, 27, 30, 31, 32.',
  },
  '14 DE ENERO': {
    jurisdicciones: ['PAMPA INALAMBRICA', 'LOS ANGELES'],
    motivo: 'Zona nueva de Ilo. P.S. Los Ángeles y C.S. Pampa Inalámbrica no se '
          + 'ponen de acuerdo sobre a quién corresponde.',
  },
};

// Se mantiene por compatibilidad: ya no se usa para estas dos zonas.
...
function asignarJurisdiccionDetalle(descripcion) {
  ...
  // ¿Es una zona de jurisdicción compartida?
  for (const zona in ZONAS_COMPARTIDAS) {
    const re = new RegExp('(^|\\s)' + zona + '(\\s|$)');
    if (re.test(t)) {
      const z = ZONAS_COMPARTIDAS[zona];
      const nombres = z.jurisdicciones.map(j => MAPA_JURISDICCIONES[j] || j);
      return {
        jurisdiccion: nombres.join(' - '),
        jurisdicciones: nombres,          // para repartir en ambos excel
        compartida: true,
        confianza: 'ALTA',
        metodo: 'JURISDICCION_COMPARTIDA',
        texto: t,
        aviso: z.motivo,
      };
    }
  ...
  // ¿Es una zona de jurisdicción compartida (en disputa)?
  for (const zona in ZONAS_COMPARTIDAS) {
    const re = new RegExp('(^|\\s)' + zona + '(\\s|$)');
    if (re.test(t)) {
      const z = ZONAS_COMPARTIDAS[zona];
      return { jurisdiccion: textoCompartida(z), confianza: 'ALTA',
               metodo: 'JURISDICCION_COMPARTIDA', texto: t,
               compartida: true, jurisdicciones: z.jurisdicciones,
               aviso: z.motivo };
    }
  }
