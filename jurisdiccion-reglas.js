...
//
// ── SALIDA POR DEFECTO ─────────────────────────────────────────────────────────
//   asignarJurisdiccion(direccion)
//     -> siempre una de las 7 jurisdicciones oficiales, o 'NO DEFINIDO'.
//
// ── ZONAS EN DISPUTA (opcional) ────────────────────────────────────────────────
//   Por defecto, Las Brisas III devuelve 'NO DEFINIDO': ambos EE.SS. la reclaman
//   y NO existe criterio por manzana para dividirla, así que no se le inventa una.
//
//   asignarJurisdiccion(direccion, { separarBrisas3: true })
//     -> devuelve 'BRISAS III' como categoría propia, para contarla aparte.
//   Úsalo solo en herramientas que necesiten separar la zona en disputa
//   (ej. la descarga por EE.SS., donde Brisas III se adjunta a Pampa y a Los Ángeles).
//   Ver ZONAS_EN_DISPUTA más abajo, y el helper zonaEnDisputa(direccion).
// ═══════════════════════════════════════════════════════════════════════════════
...
function norm(str) {
  ...
  // Ceros a la izquierda: "LT 05" -> "LT 5", "02 DE MARZO" -> "2 DE MARZO".
  // Va AQUI (normalización) y no en aplicarAlias, porque no es una
  // interpretación del texto: es la misma dirección escrita distinto.
...
function reglasRango(t) {
  ...
  // NOTA: Las Brisas III NO se resuelve aquí. Es una zona en disputa y se maneja
  // en ZONAS_EN_DISPUTA (ver más abajo). No existe criterio por manzana para dividirla.
  return null;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ZONAS EN DISPUTA (ambos EE.SS. la reclaman como suya)
// A diferencia de ZONAS_SIN_JURISDICCION, aquí SÍ hay atención: los dos
// establecimientos la consideran de su jurisdicción, así que los niños deben
// figurar en los listados de AMBOS.
//
// Comportamiento por defecto: se resuelve a una de las 7 jurisdicciones usando
// la regla de manzanas (confianza MEDIA, porque la disputa no está zanjada).
// Si llamas a la función con { separarBrisas3: true } devuelve 'BRISAS III'
// como categoría propia, para contarla aparte y adjuntarla a ambos listados.
// ═══════════════════════════════════════════════════════════════════════════════
const ZONAS_EN_DISPUTA = {
  'LAS BRISAS III': {
    etiqueta: 'BRISAS III',
    regex: /(?:LAS\s+)?BRISAS\s+III\b/,
    eess: ['C.S. PAMPA INALAMBRICA', 'P.S. LOS ANGELES'],
    motivo: 'C.S. Pampa Inalámbrica y P.S. Los Ángeles reclaman esta zona. '
          + 'Los niños deben aparecer en los listados de ambos EE.SS.'
  }
};

// ¿La dirección corresponde a una zona en disputa? Devuelve null o el objeto de la zona.
function zonaEnDisputa(descripcion) {
  const t = aplicarAlias(norm(descripcion));
  if (!t) return null;
  for (const clave in ZONAS_EN_DISPUTA) {
    if (ZONAS_EN_DISPUTA[clave].regex.test(t)) return ZONAS_EN_DISPUTA[clave];
  }
  return null;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
function asignarJurisdiccion(descripcion, opciones) {
  return asignarJurisdiccionDetalle(descripcion, opciones).jurisdiccion;
}
...
function asignarJurisdiccionDetalle(descripcion, opciones) {
  const opts = opciones || {};
  ...
  // ¿Es una zona en disputa (reclamada por dos EE.SS.)?
  for (const clave in ZONAS_EN_DISPUTA) {
    const zona = ZONAS_EN_DISPUTA[clave];
    if (!zona.regex.test(t)) continue;
    if (opts.separarBrisas3) {
      // Modo "categoría propia": para contarla aparte y adjuntarla a ambos listados.
      return { jurisdiccion: zona.etiqueta, confianza: 'ALTA',
               metodo: 'ZONA_EN_DISPUTA', texto: t, eess: zona.eess,
               aviso: zona.motivo };
    }
    // Modo por defecto (solo las 7 + NO DEFINIDO): NO DEFINIDO a propósito.
    // No existe criterio institucional para dividir esta zona, así que no se
    // le inventa una jurisdicción. Mismo tratamiento que ZONAS_SIN_JURISDICCION.
  ...
  return {
    ...
    metodo: r.metodo,
    texto: t,
    aviso: usoAlias ? `Interpretado como "${t}"` : ''
  };
}
