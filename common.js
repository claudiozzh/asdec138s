...
const msalConfig = {
  auth: {
    ...
    // OJO: tiene que ser EXACTAMENTE la misma URL que está registrada en
    // Azure AD. Azure rechaza cualquier redirectUri que no esté registrada
    // de antemano — por eso NO se arma dinámicamente según la página actual.
    // Repositorio activo desde 08/2026: e14easx62-asdec138s (el repo
    // "transito-ilo" ya no se usa, hay que quitar su URL de Azure AD).
    redirectUri: "https://claudiozzh.github.io/e14easx62-asdec138s/"
  },
...
/* Mapa de qué palabras buscar en el encabezado para cada campo que
   ...
   NOTA (corregido 08/2026): dni/cui/cnv ya NO se buscan aquí por texto
   suelto — se detectan aparte en mapearColumnas() por el sufijo real
   del encabezado "(DNI)"/"(CUI)"/"(CNV)". El patrón viejo (\"NUMERO CNV\")
   nunca hacía match con el encabezado real del gobierno, así que el CNV
   nunca se detectaba — bug real que afectaba a todo lo que usa este
   archivo (transito.html y bajas-altas.html en producción). */
const CAMPOS_PADRON = {
...
/* Encuentra, en la fila de encabezados ya detectada, la columna de
   cada campo de CAMPOS_PADRON, más dni/cui/cnv/codigoPadron aparte.
   Devuelve {campo: columna|null, ...} */
function mapearColumnas(ws, filaEncabezado) {
  const range = calcularRangoReal(ws);
  const mapa = {};
  for (const campo in CAMPOS_PADRON) mapa[campo] = null;
  mapa.eessAtencion = null; mapa.dni = null; mapa.cui = null; mapa.cnv = null; mapa.codigoPadron = null;
  const columnasVinculo = [];
  for (let c = range.s.c; c <= range.e.c; c++) {
    const cell = ws[XLSX.utils.encode_cell({ r: filaEncabezado, c })];
    if (!cell || cell.v == null) continue;
    const t = normalizarTexto(cell.v);
    // La cabecera real del padrón termina en "(DNI)", "(CUI)" o "(CNV)" —
    // nunca se detecta por texto suelto como "NUMERO CNV", que no existe así.
    if (mapa.dni === null && /\(DNI\)$/.test(t)) mapa.dni = c;
    else if (mapa.cui === null && /\(CUI\)$/.test(t)) mapa.cui = c;
    else if (mapa.cnv === null && /\(CNV\)$/.test(t)) mapa.cnv = c;
    else if (mapa.codigoPadron === null && t.includes("CODIGO DEL PADRON NOMINAL")) mapa.codigoPadron = c;
    else {
...
/* Igual que buscarDocumentoEnHoja, pero buscando por Código Padrón
   Nominal en vez de por documento — lo usan bajas-altas, tránsito y
   no-encontrados, que ya identifican al niño por su código. */
function buscarPorCodigoPadronEnHoja(ws, codigoPadron) {
  const info = encontrarInfoCabeceraCodigo(ws);
  if (!info) return null;
  const mapa = mapearColumnas(ws, info.fila);
  const range = calcularRangoReal(ws);
  ...
  const codigoNorm = String(codigoPadron).trim();
  for (let r = info.fila + 1; r <= range.e.r; r++) {
    if (val(r, info.col) !== codigoNorm) continue;
    const out = { codigoPadron: codigoNorm };
    for (const campo in mapa) out[campo] = val(r, mapa[campo]);
    return out;
  }
  return null;
...
/* Igual que buscarEnPadronDelDia, pero por Código Padrón Nominal —
   busca en los 3 archivos de un día dentro de 01_REPORTES_DIARIOS/{fecha}/
   sin que el usuario tenga que subir ningún Excel a mano. */
async function buscarPorCodigoPadronDelDia(fecha, codigoPadron) {
  const archivos = [
  ...
  for (const a of archivos) {
    ...
    const datos = buscarPorCodigoPadronEnHoja(ws, codigoPadron);
    if (datos) return { datos, archivo: a.tag, diagnostico };
    diagnostico.push(`${a.nombre}: leído bien — ese código no está ahí`);
  }
