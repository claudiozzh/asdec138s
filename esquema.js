...
/* ============================================================
   11. ESTABLECIMIENTOS DE SALUD Y SUS RESPONSABLES — v1
   ============================================================
   Un solo lugar para los 7 EE.SS. del MINSA en Ilo, sus códigos y la
   licenciada responsable de cada uno. Cualquier módulo que tenga campos
   de jurisdicción y de responsable debe llenarlos desde aquí.

   PARA ACTUALIZAR: cambia los datos en EESS_INFO y se refleja en todos
   los módulos. No copies estos datos dentro de ningún HTML.
   ============================================================ */

// Cada EE.SS. se escribe distinto según el módulo ("P.S. KENNEDY" en las reglas
// de jurisdicción, "P.S. Jhon F. Kennedy" en los selectores). Para poder
// compararlos se reduce cada nombre a una CLAVE única.
const CLAVE_JURIS = [
  ["PAMPA",      "PAMPA"],
  ["ANGELES",    "ANGELES"],
  ["KENNEDY",    "KENNEDY"],
...
// "C.S. Pampa Inalámbrica", "C.S. PAMPA INALAMBRICA" y "PAMPA" -> "PAMPA"
function claveJurisdiccion(texto) {
...
// -------------------------------------------------------------------
// DATOS DE LOS 7 EE.SS. DEL MINSA EN ILO
// Responsables del Padrón Nominal de cada establecimiento (agosto 2026).
// Los códigos IPRESS siempre tienen 8 dígitos, rellenados con ceros.
//
// El autocompletado es una SUGERENCIA: los formularios deben permitir
// editar el responsable, porque a veces se homologa con otra persona.
// -------------------------------------------------------------------
const EESS_INFO = {
  PAMPA: {
    clave: "PAMPA",
    codigo: "00034031",
    nombre: "C.S. Pampa Inalámbrica",
    nombreLargo: "C.S. PAMPA INALAMBRICA - ILO",
    responsable: { tratamiento: "Lic.", nombres: "LUZ MARINA", apellidos: "ARENAS MENA", celular: "" },
  },
  ANGELES: {
    clave: "ANGELES",
    codigo: "00007731",
    nombre: "P.S. Los Ángeles",
    nombreLargo: "P.S. LOS ANGELES - ILO",
    responsable: { tratamiento: "Lic.", nombres: "SARAHI ANGELA", apellidos: "CHINO CHIPANA", celular: "" },
  },
  KENNEDY: {
    clave: "KENNEDY",
    codigo: "00012046",
    nombre: "P.S. Jhon F. Kennedy",
    nombreLargo: "P.S. JHON F. KENNEDY - ILO",
    responsable: { tratamiento: "Lic.", nombres: "PRISSILA BRISEYRA", apellidos: "ROJAS CANAZA", celular: "" },
  },
  VARADERO: {
    clave: "VARADERO",
    codigo: "00002821",
    nombre: "P.S. Varadero",
    nombreLargo: "P.S. VARADERO - ILO",
    responsable: { tratamiento: "Lic.", nombres: "MELANY SUSANA", apellidos: "MAMANI FLORES", celular: "" },
  },
  "18MAYO": {
    clave: "18MAYO",
    codigo: "00002822",
    nombre: "P.S. 18 de Mayo",
    nombreLargo: "P.S. 18 DE MAYO - ILO",
    responsable: { tratamiento: "Lic.", nombres: "ROXANA", apellidos: "ROQUE VELASQUEZ", celular: "" },
  },
  ALTOILO: {
    clave: "ALTOILO",
    codigo: "00002819",
    nombre: "C.S. Alto Ilo",
    nombreLargo: "C.S. ALTO ILO - ILO",
    responsable: { tratamiento: "Lic.", nombres: "PAOLA OLINDA", apellidos: "ROJAS PALOMINO", celular: "" },
  },
  MIRAMAR: {
    clave: "MIRAMAR",
    codigo: "00002817",
    nombre: "C.S. Miramar",
    nombreLargo: "C.S. MIRAMAR - ILO",
    responsable: { tratamiento: "Lic.", nombres: "KATHERINN SHIRLEY", apellidos: "MARON CAYA", celular: "" },
  },
};

// Devuelve el registro del EE.SS. a partir de cualquier forma de escribirlo,
// o null si no corresponde a ninguno (por ejemplo "NO DEFINIDO").
function buscarEess(jurisdiccion) {
  const txt = String(jurisdiccion || "").trim();
  if (!txt) return null;

  // ¿Vino un código IPRESS? (con o sin los ceros de relleno)
  if (/^\d+$/.test(txt)) {
    const cod = txt.padStart(8, "0");
    for (const k in EESS_INFO) if (EESS_INFO[k].codigo === cod) return EESS_INFO[k];
    return null;
  }

  // ¿Viene el código pegado al nombre? Ej. "C.S. MIRAMAR (00002817)"
  const conCodigo = txt.match(/(\d{4,8})/);
  if (conCodigo) {
    const cod = conCodigo[1].padStart(8, "0");
    for (const k in EESS_INFO) if (EESS_INFO[k].codigo === cod) return EESS_INFO[k];
  }

  return EESS_INFO[claveJurisdiccion(txt)] || null;
}

// Lista para llenar <select> de EE.SS., con el código al costado.
// Formato: [{ valor, etiqueta, codigo, clave }]
function listaEess() {
  return Object.keys(EESS_INFO).map(k => {
    const e = EESS_INFO[k];
    return { valor: e.nombre, etiqueta: `${e.nombre} (${e.codigo})`,
             codigo: e.codigo, clave: e.clave };
  });
}

// Nombre completo de la licenciada, listo para poner en un campo de texto.
// Devuelve "" si todavía no se ha completado en EESS_INFO.
function responsableDeJurisdiccion(jurisdiccion) {
  const e = buscarEess(jurisdiccion);
  if (!e) return "";
  const r = e.responsable || {};
  // Orden: NOMBRES primero, luego APELLIDOS (así se nombran las licenciadas)
  const nombre = [r.nombres, r.apellidos].filter(Boolean).join(" ").trim();
  if (!nombre) return "";
  return [r.tratamiento, nombre].filter(Boolean).join(" ");
}

// Autocompleta el campo de responsable a partir de la jurisdicción elegida.
// No pisa lo que el usuario ya haya escrito a mano.
//   autocompletarResponsable("baja-jurisdiccion", "baja-responsable")
function autocompletarResponsable(idSelectJuris, idInputResponsable) {
  const sel = document.getElementById(idSelectJuris);
  const inp = document.getElementById(idInputResponsable);
  if (!sel || !inp) return;

  const sugerido = responsableDeJurisdiccion(sel.value);
  const previo = String(inp.value || "").trim();

  // Solo autocompleta si está vacío o si tiene el nombre de otra jurisdicción
  const esSugerenciaPrevia = inp.dataset.autocompletado === "1";
  if (previo && !esSugerenciaPrevia) return;

  inp.value = sugerido;
  inp.dataset.autocompletado = sugerido ? "1" : "";
}

// Conecta el autocompletado para que se dispare al cambiar la jurisdicción.
// Llamar una vez al cargar el módulo.
function conectarAutocompletadoResponsable(idSelectJuris, idInputResponsable) {
  const sel = document.getElementById(idSelectJuris);
  const inp = document.getElementById(idInputResponsable);
  if (!sel || !inp) return;
  sel.addEventListener("change", () => autocompletarResponsable(idSelectJuris, idInputResponsable));
  // Si el usuario escribe a mano, deja de considerarse sugerencia automática
  inp.addEventListener("input", () => { inp.dataset.autocompletado = ""; });
}

/* ============================================================
   12. TIPOS DE DOCUMENTO — terminología oficial
   ============================================================
   Únicos documentos de personas extranjeras que existen en el padrón
   nominal. NO existen PTP ni CPP.
   ============================================================ */

const TIPOS_DOCUMENTO = {
  "CARNE DE EXTRANJERIA":              { abreviatura: "C.E.",      extranjero: true },
  "PASAPORTE":                         { abreviatura: "PASAPORTE", extranjero: true },
  "DOCUMENTO DE IDENTIDAD EXTRANJERO": { abreviatura: "D.I.",      extranjero: true },
  "DNI":                               { abreviatura: "DNI",       extranjero: false },
};

// Formas de escribirlo que se aceptan y se convierten al nombre oficial.
const ALIAS_TIPO_DOCUMENTO = {
  "CE": "CARNE DE EXTRANJERIA",
  "C E": "CARNE DE EXTRANJERIA",
  "CARNE DE EXTRANJERIA": "CARNE DE EXTRANJERIA",
  "CARNET DE EXTRANJERIA": "CARNE DE EXTRANJERIA",
  "CARNE EXTRANJERIA": "CARNE DE EXTRANJERIA",
  "PASAPORTE": "PASAPORTE",
  "PAS": "PASAPORTE",
  "DI": "DOCUMENTO DE IDENTIDAD EXTRANJERO",
  "D I": "DOCUMENTO DE IDENTIDAD EXTRANJERO",
  "DI DEL EXTRANJERO": "DOCUMENTO DE IDENTIDAD EXTRANJERO",
  "DI EXTRANJERO": "DOCUMENTO DE IDENTIDAD EXTRANJERO",
  "DOCUMENTO DE IDENTIDAD EXTRANJERO": "DOCUMENTO DE IDENTIDAD EXTRANJERO",
  "DNI": "DNI",
};

// "C.E." / "ce" / "carné de extranjería" -> { nombre, abreviatura, extranjero }
// Devuelve null si no se reconoce.
function normalizarTipoDocumento(valor) {
  const v = String(valor || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .trim().toUpperCase();
  if (!v) return null;
  const clave = v.replace(/\./g, " ").replace(/\s+/g, " ").trim();
  const oficial = ALIAS_TIPO_DOCUMENTO[v] || ALIAS_TIPO_DOCUMENTO[clave];
  if (!oficial) return null;
  return { nombre: oficial, ...TIPOS_DOCUMENTO[oficial] };
}

// Opciones para un <select> de tipo de documento
function opcionesTipoDocumento(soloExtranjeros) {
  return Object.keys(TIPOS_DOCUMENTO)
    .filter(k => !soloExtranjeros || TIPOS_DOCUMENTO[k].extranjero)
    .map(k => ({ valor: k, etiqueta: TIPOS_DOCUMENTO[k].abreviatura + " — " + k }));
}
